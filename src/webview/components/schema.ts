...
export function renderSchemaBlock(idx: number, escapeHtml: (s: any) => string): string {
    return '<div class="sch-root" id="schema-root-' + idx + '"><div class="sch-toolbar"><h4 style="margin:0;font-size:14px;color:#333;">🗂 Database Schema</h4><button class="btn-primary" onclick="window.schemaLoad(' + idx + ')" style="padding:4px 10px;">Refresh</button><span id="schema-status-' + idx + '" style="font-size:11px;color:#888;margin-left:10px;"></span></div><div class="sch-content" id="schema-content-' + idx + '" style="padding:16px;font-size:12px;background:#f9f9f9;border:1px solid #ddd;border-radius:4px;margin-top:8px;max-height:400px;overflow-y:auto;"><div style="color:#888;">Click refresh to load schema...</div></div></div>';
}
